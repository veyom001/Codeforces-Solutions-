...
pair<pair<ll,ll>,ll>tree1[4*maxn];
ll arr[maxn];

pair<pair<ll,ll>,ll> merger(pair<pair<ll,ll>,int> a,pair<pair<ll,ll>,ll> b)
{
    pair<pair<ll,ll>,ll>res;

    if(a.first.first==b.first.first)
    {
        res.first=mp(a.first.first,a.first.second+b.first.second);
    ...
    res.second=gcd(a.second,b.second);
    return res;
}
...
void segmentBuild(int start,int end,int tree_start)
{
  ...
  if(start==end)
  {
        tree1[tree_start]={{arr[start],1},arr[start]};
        return;
...
tree1[tree_start]=merger(tree1[2*tree_start],tree1[2*tree_start+1]);
}
}
pair<pair<ll,ll>,ll> query(int start,int end,int left,int right,int tree_start)
{
    
  //Completely outside 
   if(start>=left&&end<=right)
  {
    return tree1[tree_start];
  }
  else

  {
    ...
    if(right<=mid)
    {
        return query(start,mid,left,right,2*tree_start);
    }

    if(mid<left)
    {
        return query(mid+1,end,left,right,2*tree_start+1);
    }

    return merger(query(mid+1,end,left,right,2*tree_start+1),query(start,mid,left,right,2*tree_start));

    //partial we can't say anything .
