...
auto ok=min_element(all(arr));
auto ok2=max_element(all(arr));

ll index1=ok-arr.begin();

ll index2=ok2-arr.begin();

ll endIndex=n-index1;
ll endIndex2=n-index2;

//cout<<endIndex+abs(endIndex2-endIndex)<<endl;

ll ans=min(index1+abs(index2-index1)+1,min(index1+1+endIndex2,min(endIndex+index2+1,endIndex+abs(endIndex2-endIndex))));
 ans=min(ans,endIndex2+abs(endIndex2-endIndex));
 ans=min(ans,index2+abs(index1-index2)+1);
cout<<ans<<endl;
